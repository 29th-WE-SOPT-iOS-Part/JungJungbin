...
class DelegatePracticeVC: UIViewController {
    static let ID = "DelegatePracticeVC"
    ...
    func configCollectionView() {
        ...
        collectionView.register(nibName, forCellWithReuseIdentifier: KeywordCVC.ID)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if nameList[indexPath.row] == "장혜령" {
            cell.presentingClosure = {
                guard let nextVC = self.storyboard?.instantiateViewController(withIdentifier: FirstVC.ID) else { return }
